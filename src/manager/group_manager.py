...
class GroupManager(StaticData):
    ...
    def block_plugin(self, module: str, group_id: int):
        ...
        参数：
            :param module: 功能模块名
            :param group_id: 群组int，None时为超级用户禁用
        """
    ...
    def get_plugin_status(self, module: str, group_id: int, is_super: bool = False) -> bool:
        """
        说明：
            获取插件开关状态, 返回False为关闭
        参数：
            :param module: 功能模块名
            :param group_id: 群组int
            :param is_super: 当为True时，检测超管是否关闭此功能
        """
    ...
    def delete_group(self, group_id: int):
        ...
        if group_id in self._data["group_manager"]:
            del self._data["group_manager"][group_id]
        if group_id in self._data["super"]["white_group_list"]:
            self._data["super"]["white_group_list"].remove(group_id)
        self.save()
