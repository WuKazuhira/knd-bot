...
class ConfigsManager:
    ...
    def __init__(self, file: Path):
        ...
        self._public_file = CONFIG_PATH / "config.yaml"
        self._simple_file = RUNTIME_CONFIG_PATH / "config.generated.yaml"
        if file:
            ...
            simple_source = self._public_file if self._public_file.exists() else self._simple_file
            if simple_source.exists():
                try:
                    with open(simple_source, "r", encoding="utf8") as f:
    ...
    def save(self, path: Union[str, Path] = None, save_simple_data: bool = False):
        ...
        if save_simple_data:
            with open(self._simple_file, "w", encoding="utf8") as f:
                _rt_yaml.dump(self._simple_data, f)
        path = path if path else self.file
        with open(path, "w", encoding="utf8") as f:
            _rt_yaml.dump(self._data, f)
...
# 配置信息管理器
Config = ConfigsManager(DATA_PATH / "config" / "plugins2config.yaml")
