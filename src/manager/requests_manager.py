...
class RequestManager(StaticData):
    ...
    def get_group_uid(self, id_: int) -> Optional[int]:
        """
        通过id获取邀请者qq号
        :param id_: id
        """
    ...
    async def show(self, type_: str) -> Optional[str]:
        ...
        for id_ in id_list:
            ...
            sex = data[id_]["sex"]
            ava = BuildImage.open(
                BytesIO(await get_user_avatar(data[id_]["id"]))
            ).resize((80, 80)).circle()
            age_bk = BuildImage.new(
                "RGBA",
                (len(str(age)) * 10, 15),
            ...
            if type_ == "private":
                ...
                A.paste(
                    Text2Image.from_text(
                        f"邀请你加入：{data[id_]['group_name']}({data[id_]['invite_group']})",
                        fontsize=12,
                        fontname="SourceHanSansCN-Regular.otf",
                        fill=(140, 140, 143),
                    ).to_image(),
            ...
            img_list.append(A)
        A = BuildImage.new("RGBA", (500, len(img_list) * 100))
        _y = 0
        for img in img_list:
            A.paste(img, (0, _y))
            _y += 100
        bk = BuildImage.new("RGBA", (A.width, A.height + 50), color="#F8F9FB")
        bk.paste(A, (0, 50), True)
        bk.draw_text(
            (15, 13),
            "好友请求" if type_ == "private" else "群聊请求",
            fontname="SourceHanSansCN-Regular.otf",
            fontsize=20,
            fill=(140, 140, 143)
        )
