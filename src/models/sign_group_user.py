...
class SignGroupUser(db.Model):
    ...
    add_probability = db.Column(
        db.Numeric(scale=3, asdecimal=False), nullable=False, default=0, server_default="0"
    )   # 额外一次性好感双倍概率
    ...
    async def ensure(
        ...
        if user and user.add_probability is None:
            await user.update(add_probability=0).apply()
            user = await query.gino.first()
