...
func TestResolveUserIDWithArg(t *testing.T) {
	...
	// 合法国服 uid
	uid, priv, errMsg := m.resolveUserID(context.Background(), mkReq("7486056827726306063", router.ServerCN))
	if errMsg != "" || priv || uid != "7486056827726306063" {
		t.Fatalf("合法 uid 参数应直接返回, got uid=%q priv=%v err=%q", uid, priv, errMsg)
	}

	// 非法 uid
	_, _, errMsg = m.resolveUserID(context.Background(), mkReq("123", router.ServerCN))
	if errMsg != errID {
