...
原 plugins/pjsk/_event_utils.py 的绘制部分。
"""
...
class EventInfoView:
    """活动信息图需要的字段（原 _models.EventInfo 的绘图子集）。"""

    _FIELDS = (
        'id', 'eventType', 'name', 'assetbundleName', 'startAt',
        'aggregateAtorin', 'aggregateAt', 'unit', 'bonusechara',
        'bonuseattr', 'music', 'cards',
    )
    ...
    def __init__(self, payload: Optional[dict] = None):
        ...
        self.eventType = self.eventType or ''
        self.name = self.name or ''
        self.assetbundleName = self.assetbundleName or ''
        self.startAt = self.startAt or ''
        self.aggregateAtorin = self.aggregateAtorin or 0
        self.aggregateAt = self.aggregateAt or ''
        self.unit = self.unit or ''
        self.bonusechara = self.bonusechara or []
        self.bonuseattr = self.bonuseattr or ''
        self.music = self.music or 0
        self.cards = self.cards or []

    @classmethod
    def payload_from_event(cls, event) -> dict:
        """把 EventInfo 对象拍平成载荷（供指令侧调用）。"""
        return {field: getattr(event, field, None) for field in cls._FIELDS}
...
async def drawevent(event, pjsk_type: int = 0):
    try:
        # 优先使用您找到的无角色背景路径
        pic = await get_context().get_asset(
    ...
    try:
        # 优先使用独立的角色无背景图片
        chara = await get_context().get_asset(
    ...
    # 绘制 Logo (位于开始时间上方)
    try:
        logo_asset = await get_context().get_asset(
    ...
    gameCharacterUnits = await get_context().async_load_master_data('gameCharacterUnits.json', pjsk_type)
    ...
    display_bonus_rows = []
    try:
        eventDeckBonuses = await get_context().async_load_master_data('eventDeckBonuses.json', pjsk_type)
        raw_unit_ids = []
...
async def draweventall(
    ...
    *args, **kwargs
    ...
    eventCards = await get_context().async_load_master_data('eventCards.json', pjsk_type)
    eventDeckBonuses = await get_context().async_load_master_data('eventDeckBonuses.json', pjsk_type)
    game_character_units = await get_context().async_load_master_data('gameCharacterUnits.json', pjsk_type)
    allcards = await get_context().async_load_master_data('cards.json', pjsk_type)
    box_label_by_event: Dict[int, str] = {}
    try:
        ban_ids = get_ban_events_id_set(pjsk_type)
        chara_box_counts: Dict[int, int] = {}
        all_events_for_box = sorted(_normalize_master_list(await get_context().async_load_master_data('events.json', pjsk_type)), key=lambda x: x.get('startAt', 0))
    ...
    if event_type != 'marathon':
        allteams = await get_context().async_load_master_data('cheerfulCarnivalTeams.json', pjsk_type)
    ...
    async def process_single_event(each):
        ...
        # 并行准备资源
        resource_tasks = []
        resource_tasks.append(get_context().get_asset(f'ondemand/event_story/{each["assetbundleName"]}/screen_image', 'banner_event_story.png', pjsk_type=pjsk_type))
        if each['eventType'] == 'cheerful_carnival':
            ...
            for team_info in event_teams:
                resource_tasks.append(get_context().get_asset(f'ondemand/event/{each["assetbundleName"]}/team_image', f'{team_info["assetbundleName"]}.png', pjsk_type=pjsk_type))
        else:
            event_teams = []
...
@register("event_info")
async def render_event_info(payload: dict) -> bytes:
    """单个活动信息图。载荷：event（EventInfoView 字段）、pjsk_type。"""
    pic = await drawevent(EventInfoView(payload.get("event")), int(payload.get("pjsk_type", 0)))
    # 原实现直接 pic.save(*.jpg)，保持不加水印、默认质量。
    return await run_pjsk_thread(image_to_jpeg, pic, quality=75, watermark=False)


@register("event_catalog")
async def render_event_catalog(payload: dict) -> Optional[bytes]:
    """活动图鉴。载荷即 event_argparse 的筛选结果 + events + display_limit。"""
    charas: List[Union[int, Tuple[int, str]]] = []
    for item in payload.get("event_charas_id") or []:
        # JSON 里元组会退化成两元列表，这里还原
        if isinstance(item, (list, tuple)) and len(item) == 2:
            charas.append((int(item[0]), str(item[1])))
        else:
            charas.append(int(item))
    pic = await draweventall(
        event_type=payload.get("event_type"),
        event_attr=payload.get("event_attr"),
        event_units_name=payload.get("event_units_name"),
        event_charas_id=charas,
        isEqualAllUnits=bool(payload.get("isEqualAllUnits", True)),
        isContainAllCharasId=bool(payload.get("isContainAllCharasId", True)),
        isTeamEvent=payload.get("isTeamEvent"),
    ...
    return await run_pjsk_thread(
        image_to_jpeg, pic.convert("RGB"), quality=70, watermark=False
    )
