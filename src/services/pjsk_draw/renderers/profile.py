...
from utils.pjsk_paths import ONDEMAND_PATH, STATIC_PATH, SUITE_PATH

from ..card import cardthumnail
from ..context import get_context
from ..honor import generatehonor
from ..primitives import get_pjsk_font, image_to_jpeg, open_pjsk_image, run_pjsk_thread
from ..profile_bg import get_user_bg_path, get_user_bg_settings
...
class ProfileView:
    """个人信息绘制需要的玩家字段（原 _models.UserProfile 的绘图子集）。

    指令侧把 UserProfile 拍平成 dict 放进载荷，这里还原成属性访问。
    """

    _FIELDS = (
        'name', 'rank', 'word', 'twitterId', 'characterId', 'characterRank',
        'userDecks', 'special_training', 'deck_master_ranks',
        'userProfileHonors', 'userHonorMissions',
        'highScore', 'clear', 'full_combo', 'full_perfect',
        'mvpCount', 'superStarCount', 'isNewData',
    )
    ...
    def __init__(self, payload: Optional[dict] = None):
        ...
        self.word = self.word or ''
        self.twitterId = self.twitterId or ''
        self.characterId = self.characterId or 0
        self.characterRank = self.characterRank or []
        self.userDecks = self.userDecks or []
        self.special_training = self.special_training or []
        self.deck_master_ranks = self.deck_master_ranks or []
        self.userProfileHonors = self.userProfileHonors or []
        self.userHonorMissions = self.userHonorMissions or []
        self.highScore = self.highScore or {}
        self.clear = self.clear or {}
        self.full_combo = self.full_combo or {}
        self.full_perfect = self.full_perfect or {}
        self.mvpCount = self.mvpCount or 0
        self.superStarCount = self.superStarCount or 0
        self.isNewData = bool(self.isNewData)

    @classmethod
    def payload_from_profile(cls, profile) -> dict:
        """把 UserProfile 对象拍平成载荷（供指令侧调用）。"""
        return {field: getattr(profile, field, None) for field in cls._FIELDS}
...
# ============ 卡组卡面获取 ============
async def _get_deck_card(profile: 'ProfileView', cards_by_id: dict, cards_list: list, index: int, pjsk_type: int) -> Optional[Image.Image]:
...
# ============ 横版布局绘制 ============
def _draw_horizontal(img: Image.Image, draw: ImageDraw.ImageDraw, profile: 'ProfileView',
...
# ============ 竖版布局绘制 ============
def _draw_vertical(img: Image.Image, draw: ImageDraw.ImageDraw, profile: 'ProfileView',
                   userid: str, isprivate: bool, pjsk_type: int,
                   deck_imgs: List[Optional[Image.Image]],
                   honor_imgs: List[Optional[Image.Image]],
                   cards_by_id: dict):
...
def _compose_profile_image_sync(profile: 'ProfileView', userid: str, isprivate: bool, pjsk_type: int,
...
# ============ 主绘图函数 ============
async def draw_new_profile(profile: 'ProfileView', userid: str, isprivate: bool, pjsk_type: int) -> Image.Image:
    """绘制新版个人信息图片"""
...
@register("profile")
async def render_profile(payload: dict) -> bytes:
    """个人信息图。载荷：profile（ProfileView 字段）、userid、is_private、pjsk_type。"""
    img = await draw_new_profile(
        ProfileView(payload.get("profile")),
