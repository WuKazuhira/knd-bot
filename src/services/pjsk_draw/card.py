"""卡牌绘制：缩略图 tile、卡面大图、查卡卡格与分组概览。

原 plugins/pjsk/_card_utils.py 的绘制部分，供各渲染器内部复用
（不是对外的渲染任务，指令侧不直接调用）。
"""
...
async def cardthumnail(cardid, istrained=False, cards=None, limitedbadge=False, fesbadge=False, pjsk_type: int = 0):
    if cards is None:
        card = get_context().master_data_by_id('cards.json', pjsk_type).get(cardid)
    else:
        card = next(
...
async def findcardsingle(
    ...
    try:
        ...
        name = ctx.getcharaname(card_obj['characterId'], gameCharacters, pjsk_type=pjsk_type)
    ...
    for cid in active_chars:
        ...
        if avatar_path.exists():
            try:
                ...
                pass
        if not avatar_drawn:
            name = ctx.getcharaname(cid, gameCharacters, pjsk_type=pjsk_type) or str(cid)
            try:
