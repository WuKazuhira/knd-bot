...
原 plugins/pjsk/_profile_header.py 的绘制部分。指令侧只把
:func:`plugins.pjsk._profile_header.build_header_payload` 产出的 dict 放进载荷，
这里用 :meth:`PjskHeaderData.from_payload` 还原。
"""
...
from .primitives import get_pjsk_asset_cached, get_pjsk_font
...
class PjskHeaderData:
    ...
    @classmethod
    def from_payload(cls, payload: Optional[dict]) -> "PjskHeaderData":
        payload = payload or {}
        return cls(
            userid=str(payload.get('userid', '')),
            name=payload.get('name') or '???',
            rank=payload.get('rank') or 0,
            is_private=bool(payload.get('is_private')),
            user_decks=list(payload.get('user_decks') or []),
            special_training=list(payload.get('special_training') or []),
            user_profile_honors=list(payload.get('user_profile_honors') or []),
            user_honor_missions=list(payload.get('user_honor_missions') or []),
            suite_update_time=payload.get('suite_update_time'),
        )
...
async def _load_cutout(data: PjskHeaderData, card_asset_map: Optional[Dict[int, str]], pjsk_type: int) -> Optional[Image.Image]:
    if not data.user_decks:
        return None
    get_asset = get_context().get_asset
    try:
        asset_name = card_asset_map.get(data.user_decks[0], '') if card_asset_map else ''
        if not asset_name:
            return None
        suffix = 'after_training' if (data.special_training and data.special_training[0]) else 'normal'
        cutout = await get_asset(
