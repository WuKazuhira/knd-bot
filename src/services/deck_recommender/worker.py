...
try:
    ...
    class _SetProcTitleFallback:
        @staticmethod
        def setproctitle(_: str) -> None:
            return None

    setproctitle = _SetProcTitleFallback()
...
class Worker:
    ...
    def recommend(self, region: str, options: dict, userdata_hash: str) -> dict:
        ...
        try:
            ...
            self.log(
                f"进入推荐流程 region={region} "
                f"masterdata_version={self.masterdata_version.get(region)} "
                f"musicmetas_update_ts={self.musicmetas_update_ts.get(region)}"
            )

            if not self.masterdata_version.get(region) or not self.musicmetas_update_ts.get(region):
                self.log(
                    f"数据未初始化: masterdata_version={self.masterdata_version.get(region)} "
                    f"musicmetas_update_ts={self.musicmetas_update_ts.get(region)}"
                )
                return {
                    'status': 'error',
...
class WorkerContext:
    ...
    def worker_loop(worker: Worker, task_queue: Queue, result_queue: Queue):
        setproctitle.setproctitle(f'kndbot-deckrec-worker-{worker.worker_id}')
    ...
    def init_workers(cls, worker_num: int):
        if mp.current_process().name == 'MainProcess':
            setproctitle.setproctitle('kndbot-deckrec-main')
        cls.all_workers = {}
