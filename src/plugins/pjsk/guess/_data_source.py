...
async def getRandomChart(pjsk_type: int = 0) -> Tuple[int, str]:
    ...
    server_name = SERVER_MAP.get(pjsk_type, 'jp')
    if not (data_path / server_name / path / 'master.jpg').exists():
        await moe2img(musicid, 'master', pjsk_type=pjsk_type)
...
# 裁剪函数
def cutChart(musicid: int, qunnum: int, is_tip: bool = False, pjsk_type: int = 0) -> Union[Tuple[Path, Path], Path]:
    """
    裁剪谱面图片
    """
    server_name = SERVER_MAP.get(pjsk_type, 'jp')
...
def cutLyrics(musicid:int, asset: str, qunnum: int, pjsk_type: int = 0) -> Tuple[str, Path]:
    """
    裁切歌词
    """
    server_name = SERVER_MAP.get(pjsk_type, 'jp')
