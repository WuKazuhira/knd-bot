...
def getSongLevel(musicid: int, diff: str = 'master', pjsk_type: int = 0) -> str:
    """
...
def getSongNoteCount(musicid: int, diff: str = 'master', pjsk_type: int = 0) -> str:
    """
...
def getSongSinger(musicid: int, pjsk_type: int = 0) -> str:
...
def getSongAuthor(musicid: int, pjsk_type: int = 0) -> str:
...
def getSongLyrics(musicid: int, pjsk_type: int = 0):
    """
    从歌词文件中获取连续的两行，忽略空行。
    """
...
def getCharaUnit(charaid: int, pjsk_type: int = 0) -> str:
...
def getCharaInfo(charaid: int, pjsk_type: int = 0) -> str:
    characterProfiles = load_master_data('characterProfiles.json', pjsk_type)
...
def getCharaBirth(charaid: int, pjsk_type: int = 0) -> str:
    data = load_master_data('characterProfiles.json', pjsk_type)
    profile = data[str(charaid)]
    birth_month = profile['birthday'].split('月')[0]
    return f'此角色的生日在{birth_month}月'


def getCharaFeature(charaid: int, pjsk_type: int = 0) -> str:
    data = load_master_data('characterProfiles.json', pjsk_type)
    profile = data[str(charaid)]
    feature_lst = []
    [
