...
async def parse_html(uri):
    ...
    template = "{num}. {check}"
    try:
        temp_rep = await AsyncHttpx.get(uri)
        flag = True if 'iframe' in temp_rep.text else False
        if flag:
            soup = BeautifulSoup(temp_rep.text, 'lxml')
            target = soup.select('body > div.mianbody > div > div.leftB > div.lB2 > iframe')
            for i in target:
                uri = i['src']
                break
            else:
