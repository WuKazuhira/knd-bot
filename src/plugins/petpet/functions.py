...
async def operations(
        ...
        opt: str = RegexArg("opt"),
        arg: str = Arg()
):
    help_msg = '请发送"改图指令"查看支持的指令'
    if not opt:
        return help_msg
    opt = opt.strip()
    isreverse = True if opt == "倒放" else False
    ...
    def make(img: BuildImage) -> BuildImage:
        if opt == "倒放":
            ...
            angle = int(arg) if arg and arg.isdigit() else 90
            frame = img.rotate(int(angle))
        elif opt == "反相":
            img = img.convert("RGB")
...
def crawl(img: BuildImage = UserImg(), arg=NoArg()):
    num = 0
...
def punch(img: BuildImage = UserImg(), arg=NoArg()):
    ...
    locs = [
        ...
        (10, 10), (20, 20), (10, 10), (0, 0), (-10, -10), (-20, 0), (-30, 10)
    ]
    # fmt: on
    for i in range(13):
...
def zhengjian(imgs: List[BuildImage] = UserImgs(1, 2), arg=NoArg()):
    base_img_size = (700, 700)
    face_img_size = (200, 200)
    if len(imgs) == 1:
        base_img = imgs[0].convert("RGBA").resize(base_img_size, inside=True, keep_ratio=True, bg_color="white")
        img = None
    else:
        base_img = imgs[0].convert("RGBA").resize(base_img_size, inside=True, keep_ratio=True, bg_color="white")
        img = imgs[1].convert("RGBA").resize(face_img_size, inside=True, keep_ratio=True, bg_color="white")

    if img:
        base_face_img = base_img.convert("RGBA")
        face_img = img.convert("RGBA")
    else:
        base_face_img = base_img.convert("RGBA")
        face_img = base_img.convert("RGBA").resize(face_img_size, inside=True, keep_ratio=True, bg_color="white")
    fg_img = load_image("zhengjian/fg.png")
    bg_img = load_image("zhengjian/bg.png")

    id_img = load_image("zhengjian/id.png")
    mask_img = load_image("zhengjian/mask.png").convert('L')

    # 手指区域图片
    base_face_img = base_face_img.crop((65, 0, 700, 700))
    bg_img.paste(base_face_img, (204, 65))
    # 前景图片
    bg_img.paste(fg_img, (0, 0), alpha=True)  # fixed
    # 证件区域图片
    id_img.paste(face_img, (76, 464))

    fin_img = bg_img.composite(id_img, mask_img)
    fin_img = fin_img.crop((0, 68, 712, 687))
    frame = fin_img
    return frame.save_jpg()
...
def mywife(
    ...
    rule = r"[^\s\w]"
    if re.search(rule,ta) or re.search(rule,name):
        return None
...
def walnutzoom(img: BuildImage = UserImg(), arg=NoArg()):
    # fmt: off
    locs = (
    ...
    def maker(i: int) -> Maker:
        def make(img: BuildImage) -> BuildImage:
            frame = load_image(f"walnutzoom/{i}.png")
            x, y, w, h = locs[seq[i]]
...
def addiction(img: BuildImage = UserImg(), arg: str = Arg()):
...
def trance(img: BuildImage = UserImg(), arg: str = NoArg()):
