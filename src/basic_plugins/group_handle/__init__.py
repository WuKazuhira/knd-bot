...
Config.add_plugin_config(
    "invite_manager", "welcome_message", [f"欢迎新人，这里是{NICKNAME}，发送/help sekai获取食用说明~"], help_="有新人进群的欢迎消息"
)
Config.add_plugin_config(
...
async def _(bot: Bot, event: GroupDecreaseNoticeEvent):
    ...
    if _kick_flmt.check(event.group_id):
        ...
        avatar = image(f"https://q.qlogo.cn/headimg_dl?dst_uin={event.user_id}&spec=640")
        rst = ""
        if event.sub_type == "leave":
            rst = f"啊...{user_name}(Q{event.user_id})退群了..."
        if event.sub_type == "kick":
            ...
            rst = f"{user_name}(Q{event.user_id})被{operator_name}送走了呢"
        try:
            await group_decrease_handle.send(f"[[_task|refund_group_remind]]{rst}" + avatar)
        except ActionFailed:
            return
