...
async def init_plugins_settings(data_path: Path):
    ...
    """
    plugins2settings_file = data_path / "config" / "plugins2settings.yaml"
    ...
    for x in plugins2settings_manager.keys():
        try:
            _plugin = get_plugin_by_module_name(x)
    ...
    # 设置插件文件说明
    _data = _rt_yaml.load(open(plugins2settings_file, encoding="utf8"))
