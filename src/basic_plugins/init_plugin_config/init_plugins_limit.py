...
def init_plugins_cd_limit(data_path: Path):
    """
    加载 cd 限制
    """
    plugins2cd_file = data_path / "config" / "plugins2cd.yaml"
    ...
    with open(plugins2cd_file, "w", encoding="utf8") as wf:
        _rt_yaml.dump(_tmp_data, wf)
    # 写入配置项说明
    _data = _rt_yaml.load(open(plugins2cd_file, encoding="utf8"))
    _data["PluginCdLimit"].yaml_set_start_comment(
        """# 需要cd的功能
# 自定义的功能需要cd也可以在此配置
# key：模块名称
# cd：cd 时长（秒）
# count_limit：cd内最多触发指令的次数上限
# status：此限制的开关状态
...
# rst 为 "" 或 None 时则不回复
# rst示例："别急，[uname]/[nickname]你要等[cd]秒后再用[at]"
# rst回复："别急，豆腐人/小杯面你要等[cd]秒后再用[at]"
#           用户昵称↑ ↑昵称系统的昵称  艾特用户↑""",
        indent=2,
    )
    with open(plugins2cd_file, "w", encoding="utf8") as wf:
        _rt_yaml.dump(_data, wf)
...
def init_plugins_block_limit(data_path):
    ...
    加载阻塞限制
    """
    plugins2block_file = data_path / "config" / "plugins2block.yaml"
    ...
    with open(plugins2block_file, "w", encoding="utf8") as wf:
        _rt_yaml.dump(_tmp_data, wf)
    _data = _rt_yaml.load(open(plugins2block_file, encoding="utf8"))
...
def init_plugins_count_limit(data_path):
    ...
    加载次数限制
    """
    plugins2count_file = data_path / "config" / "plugins2count.yaml"
    ...
    with open(plugins2count_file, "w", encoding="utf8") as wf:
        _rt_yaml.dump(_tmp_data, wf)
    _data = _rt_yaml.load(open(plugins2count_file, encoding="utf8"))
