...
def init_plugins_config(data_path: Path):
    """
    初始化插件数据配置
    """
    # 从文件内读取旧配置
    plugins2config_file = data_path / "config" / "plugins2config.yaml"
    ...
    for matcher in get_matchers():
        if matcher.plugin_name not in _tmp:
            ...
            try:
                ...
                continue
            # 插件配置版本更新 或 版本为None 或 插件配置尚未被载入
            if (
    ...
    if not Config.is_empty():
        Config.save()
        _data = _rt_yaml.load(open(plugins2config_file, encoding="utf8"))
        ...
        # 开始读取用户配置
        user_config_file = CONFIG_PATH / "config.yaml"
        _data = {}
        _tmp_data = {}
        ...
        try:
            ...
            with open(data_path / "config" / "config.generated.yaml", "w", encoding="utf8") as wf:
                _rt_yaml.dump(_data, wf)
