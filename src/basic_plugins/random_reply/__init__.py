...
def _filter_sendable_message(message: Union[Message, MessageSegment, str]) -> Message:
    """过滤 OneBot v11 send_msg 不稳定/不支持的消息段，避免随机回复把 matcher 打崩。"""
    msg = message if isinstance(message, Message) else Message(message)
    safe = Message()
    unsupported = []
    for seg in msg:
        if seg.type in {"text", "at", "face", "image", "record", "reply"}:
            safe.append(seg)
        else:
            unsupported.append(seg.type)
    if unsupported:
        logger.warning(f"随机消息模块跳过不支持的消息段: {unsupported}, raw={msg!r}")
    return safe
...
async def send_msg(matcher: Matcher, res: Union[Message, MessageSegment, str]):
    ...
    for index, per in enumerate(fin_msg, 1):
        if not per:
            continue
        safe_msg = _filter_sendable_message(per)
        if not safe_msg:
            continue
        try:
            await matcher.send(safe_msg)
        except ActionFailed as e:
            logger.warning(f"随机消息发送失败，已跳过: {e}; message={safe_msg!r}")
