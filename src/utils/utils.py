...
def get_matcher_plugin(matcher):
    """
    根据 matcher 获取真实插件对象
    """
    for key in (
        getattr(matcher, 'plugin_id', None),
        getattr(matcher, 'module_name', None),
        getattr(matcher, 'plugin_name', None),
    ):
        if not key:
            continue
        plugin = nonebot.plugin.get_plugin(key)
        if plugin:
            return plugin
    return None


def get_plugin_by_module_name(module_name: str):
    """
    根据模块名获取真实插件对象
    """
    if not module_name:
    ...
    for matcher in get_matchers():
        plugin = get_matcher_plugin(matcher)
        if not plugin:
            continue
        module = getattr(plugin, 'module', None)
        module_key = getattr(module, '__name__', '') if module else ''
        plugin_id = getattr(plugin, 'plugin_id', '')
        plugin_name = getattr(plugin, 'name', '')
        if module_name in {plugin_id, plugin_name, module_key}:
            return plugin
        if module_key.endswith(f'.{module_name}') or plugin_id.endswith(f':{module_name}'):
            return plugin
    return None
