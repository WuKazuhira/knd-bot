...
def image(
    ...
    if isinstance(file, Path):
        if file.exists():
            return MessageSegment.image(file.read_bytes())
        ...
        target_file = IMAGE_PATH / path / file if path else IMAGE_PATH / file
        if target_file.exists():
            return MessageSegment.image(target_file.read_bytes())
        else:
...
def reply(id: int) -> MessageSegment:
    """
    说明：
        生成一个 MessageSegment.reply 消息
    参数：
        :param id: 消息id
    """
    return MessageSegment.reply(id)
...
def record(
    voice_name: Union[str, Path, bytes] = None,
    path: str = None
) -> Union[MessageSegment, str]:
    """
    说明：
    ...
    参数：
        :param voice_name: 音频文件名称/路径/字节，默认在 resources/record 目录下
        :param path: 音频文件路径，默认在 resources/record 目录下
    """
    if isinstance(voice_name, Path):
        if voice_name.exists():
            return MessageSegment.record(voice_name.read_bytes())
        logger.warning(f"语音 {voice_name.absolute()}缺失...")
        return ""
    elif isinstance(voice_name, bytes):
    ...
    if file.exists():
        return MessageSegment.record(file.read_bytes())
    else:
