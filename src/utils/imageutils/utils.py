...
KND_WATERMARK_TEXT = "DESIGNED by KNDBOT in California"


def _watermark_font(size: int):
    for name in ("FOT-RodinNTLGPro-DB.ttf", "SourceHanSansCN-Medium.otf"):
        try:
...
def add_kndbot_watermark(pic: Image, text: str = KND_WATERMARK_TEXT) -> Image:
    """给静态 PIL 图片右下角添加 KNDBOT 水印。"""
    if not isinstance(pic, Image.Image) or min(pic.size) < 120:
        return pic

    out = pic.copy()
    draw = ImageDraw.Draw(out)
    font_size = max(10, min(18, int(min(out.size) * 0.018)))
    font = _watermark_font(font_size)
    bbox = draw.textbbox((0, 0), text, font=font)
    text_w = bbox[2] - bbox[0]
    text_h = bbox[3] - bbox[1]
    pad = max(10, int(min(out.size) * 0.012))
    x = max(pad, out.width - text_w - pad)
    y = max(pad, out.height - text_h - pad)
