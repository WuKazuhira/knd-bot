...
class Char:
    def __init__(
        ...
        self.width = self.width if self.width else self.fontsize
        self.height = self.height if self.height else self.fontsize
        if self.font.valid_size:
    ...
    def draw_on(self, img: IMG, pos: PosTypeInt):
        if self.font.valid_size:
            ...
            new_img = new_img.resize(
                (int(self.width), int(self.height)), resample=Image.Resampling.LANCZOS
            )
...
class Text2Image:
    ...
    def from_text(
        ...
        :参数:
          ...
          * ``stroke_fill``: 描边颜色
          * ``fontname``: 指定首选字体
          * ``fallback_fonts``: 指定备选字体，仅当某字在fontname字体下不存在时有效
          * ``ischeckchar``: 是否检查每个字是否存在，仅在fontname存在时有效
        """
        lines: List[Line] = []
        chars: List[Char] = []
        ...
        for char in text:
            ...
            try:
                ...
                font = Font.find(fontname, style, weight)
                if ischeckchar and not font.has_char(char):
                    raise FontNotFoundError
    ...
    def size(self) -> Tuple[int, int]:
        if not self.lines:
            width = height = 0
        width = max([line.width for line in self.lines])
        height = (
